...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult, BOOL parentContent) {
    ...
    if (currentNode->content && currentNode->content != (xmlChar *) -1) {
        ...
        if ([[resultForNode objectForKey:@"nodeName"] isEqual:@"text"] && parentResult) {
            if (parentContent) {
                [parentResult setObject:[currentNodeContent stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]
                                 forKey:@"nodeContent"];
                return nil;
            }
    ...
    if (currentNode->content) {
        ...
        if (currentNode->type == XML_TEXT_NODE) {
            if (currentNode->parent->type == XML_ELEMENT_NODE) {
                ...
                if ([parentResult[@"nodeContents"] count] > 0) {
                    [contents setArray:parentResult[@"nodeContents"]];
                }
                [contents addObject:currentNodeContent];
                [parentResult setObject:contents forKey:@"nodeContents"];
//                NSLog(@"content: %@",currentNodeContent);
                return nil;
            }
